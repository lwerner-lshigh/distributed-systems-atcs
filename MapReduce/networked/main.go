		...
		Commands: []*cli.Command{
			{
				...
				Usage:   "Host a MapReduce server",
				Flags: []cli.Flag{
					&cli.IntFlag{
						Name:    "minWorkers",
						Aliases: []string{"mw"},
						Value:   3,
						Usage:   "Minimum number of workers to be able to start the map-reduce job.",
					},
					&cli.StringSliceFlag{
						Name:      "file",
						Required:  true,
						TakesFile: true,
						Usage:     "A file to go through for map-reduce. Can be used multiple times to specifiy many files.",
					},
				},
				Action: func(c *cli.Context) error {
					server := new(servers.CoordinatorServer)
					server.Files = append(server.Files, c.StringSlice("file")...)
					workers := c.Int("minWorkers")
			...
			},
			{
				Name:    "client",
				Aliases: []string{"c"},
				Usage:   "Run a MapReduce client",
				Flags: []cli.Flag{
					&cli.StringFlag{
						Name:     "server",
						Aliases:  []string{"s"},
						Required: true,
						Usage:    "The mapreduce server to connect to",
					},
				},
				Action: func(c *cli.Context) error {
					...
					clientServer.CoordinatorServer = c.String("server")
